...
def build_strategy_definitions() -> list[StrategyDefinition]:
    return [
        ...
        StrategyDefinition(
            ...
            label="目标收益带宽定投",
            description="按月以预期年化 10%、基准定投 1000 美元、波动带 300 美元动态调整投入，价格高时少投，价格低时多投。",
            simulator=lambda df: simulate_target_value_band_strategy(
                df,
                key="target_value_band_dca",
                label="目标收益带宽定投",
                description="按标准价值平均法，以预期年化 10%、基准定投 1000 美元、波动带 300 美元动态调整投入；目标市值沿固定复利路径增长，偏低时多补，偏高时少投。",
